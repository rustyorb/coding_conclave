...
1. Enter-to-send: verify the composer Enter key submits across real keyboards/browsers (synthetic key events in automation did not trigger `requestSubmit`; likely automation artifact, needs a human-adjacent check). `public/app.js` keydown handler.
2. Legacy-noise cleanup UX: the live `.conclave/state.json` has ~52 tasks, many legacy `origin: 'message'` chat noise. Archive works per-task; consider a "bulk archive legacy chat tasks" action (PRD §37.3) with a confirm step.
3. `/api/state` payload is >3.6 MB on the live room and is re-fetched on every SSE ping; executions carry full output. Slice `output` out of the state endpoint (serve via `/api/executions/:id/output`) or truncate in the projection. Measure before/after.
4. Board keyboard alternatives (FR-TASK-014): lane-move via menu/keyboard is not implemented yet (no drag either, so no regression, but Ready→Inbox etc. transitions need explicit controls beyond the current action buttons).
5. E2E pass on the Runs page with a real approved command (`node --version`) and a real agent chat turn — confirm output streaming, cancel, and exit-code rendering.
6. Accessibility spot-check: approvals drawer focus management, `aria-expanded` on the Approvals button, turn-chip cancel buttons reachable by keyboard.
7. README still describes the old single-page tabs — update the UI section (README has other uncommitted edits from a prior run; reconcile them).
8. **BIG ONE — autopilot/scheduler re-integration.** `origin/main` (PR #1, a loop session) built an autopilot policy engine + dependency scheduler on the pre-separation server. The merge kept its compatible pieces (process-manager reserve/release + cancel reasons, broadened redaction, store hardening, CSRF guards, pure `src/lib/policy.js` + `src/lib/scheduler.js` modules and `test/policy.test.js`), but its server wiring (`schedulePass`, `retryTask`, auto-approval, autopilot UI panel) was deliberately deferred: it assumes `@mention → task`, `accessMode` on messages, and concurrent runs per agent — all of which contradict the chat-turn model and PRD §4.7. Re-integrate it onto the current server: wire `selectStartable`/`validateDependencies` into `startQueuedTasks`, add the policy endpoints + autopilot panel (Approvals drawer or a Settings page), keep one-run-per-agent and chat turns intact, then port `deferred-tests/*.integration.js` to the new model and move them back into `test/`. See `deferred-tests/README.md`.

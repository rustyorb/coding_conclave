...
10. NEW (from adversarial review, 2026-07-14) — **loopback API auth token**: mutating routes (`/api/policy`, `/api/approvals/:id`, …) are reachable by any local process, including a running agent, which could author the policy that governs it. Add a per-boot bearer token printed at startup and required by the browser client on mutating routes; documented as a known limitation in README until fixed.
